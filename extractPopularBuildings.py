...
with open('/data/movement.csv', 'rb') as csvfile:
    with open('/data/clean_movement.csv', 'wb') as writecsv:
        ...
        for row in rows:
            ...
            if buildingName in popBuildings and int(row['userId']) in popU and reqCount < 100000:
                writer.writerow([row['timestamp'], row['userId'], row['AP']])
                reqCount += 1



print reqCount
#sortedUsrs = sorted(popUsers.items(), key=operator.itemgetter(1))
#print dict(sortedUsrs[-300:]).keys()